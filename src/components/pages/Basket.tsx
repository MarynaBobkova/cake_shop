...
const Basket: React.FC<Props> = ({
  ...
  return (
    <div style={{ width: "60%", margin: "0 auto" }}> {/* Див с стилями ширины и отступа */}
    <h1 className="display-5 text-center header1">Your Basket</h1> {/* Заголовок корзины */}
    {selectedProducts.length === 0 ? ( // Проверяем, пуста ли корзина
      ...
      <div>
        {selectedProducts.map((product, index) => ( // Проходимся по каждому продукту в корзине
          <div key={index} className="row mb-3"> {/* Див с классом строки и отступом */}
            <div className="col">{product.title}</div> {/* Отображаем название продукта */}
            <div className="col">{product.quantity}</div> {/* Отображаем количество продукта */}
            <div className="col">
              Price:{" "}
        ...
        <div>Total Price: {calculateTotalPrice()}</div> {/* Отображаем общую стоимость продуктов в корзине */}
        <button className="btn btn-primary">Checkout</button> {/* Кнопка оформления заказа */}
      </div>
    )}
  </div>
);
